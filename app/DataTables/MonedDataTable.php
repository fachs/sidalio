...
class MonedDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->editColumn('total_harga', function ($row) {
            $penerimaan = Penerimaan::find($row->obat_id);
            return 'Rp ' . number_format($penerimaan->total_harga, 0, ',', '.');;
        })
        // ->editColumn('qty', function ($row) {
        //     $penerimaan = Penerimaan::find($row->obat_id);

        //     if ($penerimaan) {
        //         $mutasi = Mutasi::find($row->obat_id);

        //         if ($mutasi) {
        //             // Jika mutasi ditemukan, lakukan perhitungan yang valid
        //             $stock = $penerimaan->qty - $mutasi->jumlah;
        //         } else {
        //             // Jika tidak ada mutasi, maka stock = qty penuh
        //             $stock = $penerimaan->qty;
        //         }
        //     } else {
        //         // Jika tidak ada penerimaan, stock = 0
        //         $stock = 0;
        //     }
        ...
        ->editColumn('countdown', function ($row) {
            ...
            return floor($daysRemaining) . ' hari';
        })
        ...
        //     ]);
        // })
        ->rawColumns(['countdown','created_at']);
    }
    ...
    public function html(): HtmlBuilder
    {
        return $this->builder()
                ...
                ->addColumn(['data' => 'qty', 'title' => __('Stock'), 'orderable' => true, 'searchable' => true,])
                ->addColumn(['data' => 'exp_date', 'title' => __('ED'), 'orderable' => true, 'searchable' => true,])
                ...
                ->addColumn(['data' => 'countdown', 'title' =>  __('Menuju ED'), 'orderable' => true, 'searchable' => true])
